...
public class BaseWidget extends Module implements IBaseWidget
{
    ...
    private function configResultHandler(event:ResultEvent):void
    {
        try
        {
			...
			//Fabian: convertir el resultado de la lectura del archivo de configuración en una 
			//variable tipo Object que es más facil de utilizar
			try
			{
				var xmlDoc:XMLDocument = new XMLDocument(configXML);
				var decoder:SimpleXMLDecoder = new SimpleXMLDecoder(true);
				configObject = decoder.decodeXML(xmlDoc);
			}catch(e:Error)
			{
				showError("El archivo de configuración no pudo ser interpretado correctamente.\n" + e.message);
			}
			//Fin modificación
	...
	/**
	 * Atributo para la clase BaseWidget que contiene la información del archivo de configuración
	 * como tipo Object. por defecto la clase BaseWidget posee el atributo <code>configXML</code> que contiene la misma
	 * información pero en formato XML.
	 */
